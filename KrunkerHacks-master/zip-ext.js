...
class Utilities {
    ...
    onLoad() {
        ...
        this.newFeature('AutoAim', "p", ['Off', 'Aim Assist', 'Aim Bot', 'Trigger Bot']);
        this.newFeature('AutoBhop', "o", ['Off', 'Auto Jump', 'Auto Slide']);
        this.newFeature('EspMode', "i", ['Off', 'Full', '2d', 'Walls']);
        this.newFeature('NoDeathDelay', "u", []);
    ...
    resetSettings() {
        if (confirm("Are you sure you want to reset all your ugly settings? This will also refresh the page")) {
			...
			if (ui.frustum.containsPoint(target)) {
				...
				if (this.settings.espMode > 0 && this.settings.espMode != 3) {
					...
					if (this.settings.espMode === 1) {
						...
						this.ctx.restore();
						let grad2 = this.gradient(0, 0, meas[4] * 5, 0, ["rgba(148, 0, 211)", "rgba(0, 0, 255)"]);
						let padding = 2;
						//if (entity.rankIcon && entity.rankIcon.complete) {
						//	let grad = this.gradient(0, 0, (meas[4] * 2) + meas[3] + (padding * 3), 0, ["rgba(148, 0, 211)", "rgba(0, 0, 255)"]);
    ...
    drawMenu()  {
        ...
        this.rect(X, Y, 0, 0, width, height, 'rgba(0, 0, 255, 0.42)', true);
        this.rect(X, Y, 0, 0, width, height, 'rgba(0, 0, 255, 0.42)', true);
        this.text("Uglyz", "20px GameFont", "#ff0000", width / 1.72 - this.getTextMeasurements(["Uglyz"]) - X / 1.72, Y + 40);
        this.rect(X + 10, Y + 60, 0, 0, width -20, height -70, '#rgba(0, 0, 0, 0.7', true);
